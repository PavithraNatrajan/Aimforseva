...
Filename : generalDonationCtrl.js 
Purpose / Functionality: Performing the CRUD operations for Data Management from client-side
Author : Ankit(id:T0002)
*/

angular.module('generaldonationCtrl', ['angularUtils.directives.dirPagination', 'commonServices'])
    .controller('donationCtrl', function($http, $state, $timeout, $scope, commonFactory) {
        var app = this;
        var url = 'generaldonation'; //calling the url to fetch the data
       ...
       var allProject = function() {
        ...
        $scope.generalDonor = function(regData) {
            $scope.generalDonor = [];

            commonFactory.generalDonorById(url, regData).then(function successCallback(response) {
                if (response.data.success == "0") {
                    swal("Receipt number is not define ?", {
                        icon: "warning",
                    });
                } else {
